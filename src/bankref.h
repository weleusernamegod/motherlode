// BANKREF_EXTERN(a_button)
BANKREF_EXTERN(character)
BANKREF_EXTERN(drill_h)
BANKREF_EXTERN(drill_v)
BANKREF_EXTERN(fuel_display)
BANKREF_EXTERN(game_over_skull)
BANKREF_EXTERN(highlight_frame_bank2)
BANKREF_EXTERN(highlight_frame_bank3)
BANKREF_EXTERN(highlight_frame)
BANKREF_EXTERN(loading_screen)
BANKREF_EXTERN(main_menu_buttons)
BANKREF_EXTERN(nav)
BANKREF_EXTERN(ore_tiles)
BANKREF_EXTERN(powerup_frame)
BANKREF_EXTERN(progressbar)
BANKREF_EXTERN(prop)
BANKREF_EXTERN(rover_eye)
BANKREF_EXTERN(rover)
BANKREF_EXTERN(sell_frame)
BANKREF_EXTERN(splashscreen)
BANKREF_EXTERN(station_powerup)
BANKREF_EXTERN(station_save)
BANKREF_EXTERN(station_sell)
BANKREF_EXTERN(station_upgrade)
BANKREF_EXTERN(station)
BANKREF_EXTERN(tracks)
BANKREF_EXTERN(upgrade_frame)
BANKREF_EXTERN(upgrade_tick)
BANKREF_EXTERN(upgrade_tiles)
BANKREF_EXTERN(warning_cargo)
BANKREF_EXTERN(warning_fuel)
BANKREF_EXTERN(splashscreen_screen)
BANKREF_EXTERN(SFX_down)
BANKREF_EXTERN(SFX_up)
BANKREF_EXTERN(SFX_level_up)
BANKREF_EXTERN(SFX_exp_up)
BANKREF_EXTERN(SFX_blep)
BANKREF_EXTERN(SFX_explosion)
BANKREF_EXTERN(SFX_hit)
BANKREF_EXTERN(SFX_secret)
BANKREF_EXTERN(SFX_soft_hit)
BANKREF_EXTERN(SFX_glitch)

BANKREF_EXTERN(bank_gameloop)
BANKREF_EXTERN(bank_gameover)
BANKREF_EXTERN(bank_main_menu)
BANKREF_EXTERN(bank_map)
BANKREF_EXTERN(bank_player)
BANKREF_EXTERN(bank_powerup)
BANKREF_EXTERN(bank_sell)
BANKREF_EXTERN(bank_upgrade)
BANKREF_EXTERN(bank_loading)


